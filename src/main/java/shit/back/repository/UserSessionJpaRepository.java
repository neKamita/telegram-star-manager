...
public interface UserSessionJpaRepository extends JpaRepository<UserSessionEntity, Long> {
    ...
    List<UserSessionEntity> findUsersStuckInState(@Param("state") SessionState state, @Param("cutoff") LocalDateTime cutoff);
    ...
    // Время жизни сессий
    @Query("SELECT AVG(EXTRACT(EPOCH FROM (s.updatedAt - s.createdAt))/3600) FROM UserSessionEntity s WHERE s.state = 'COMPLETED'")
