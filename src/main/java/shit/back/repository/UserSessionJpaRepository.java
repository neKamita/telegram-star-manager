...
public interface UserSessionJpaRepository extends JpaRepository<UserSessionEntity, Long> {
        ...
        @Query("SELECT new shit.back.model.UserCountsBatchResult(" +
                        "COUNT(*), " +
                        "COUNT(CASE WHEN s.isActive = true AND s.lastActivity > :activeThreshold THEN 1 END), " +
                        "COUNT(CASE WHEN s.isActive = true AND s.lastActivity > :onlineThreshold THEN 1 END)) " +
